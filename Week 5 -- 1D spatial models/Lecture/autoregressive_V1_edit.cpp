...
Type objective_function<Type>::operator() ()
{
  ...
  if( Options_vec(0)==0 ){
    ...
    for(int s=1; s<n_s; s++) {
        jnll_comp(1) -= dnorm( epsilon_s(s), rho*epsilon_s(s-1), pow(sigma2,0.5), true );
    }
  }
  ...
  if( Options_vec(0)==2 ){
    ...
    REPORT( Cov_ss );
    jnll_comp(1) += MVNORM(Cov_ss)( epsilon_s ); // called from namespace density
    // Mvnorm takes a covarainace matrix and returns a function
  }
