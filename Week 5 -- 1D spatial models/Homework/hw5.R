...
Sim_Fn <- function(n_i=1000, Scale=2, logSD_spatial=0.1, L0=10, Linf_0=100, 
                   beta_y=0.03, growth_rate=0.1, mortality_rate=growth_rate*1.6,
                   logSD_resid=0.05, plot=FALSE, print=NULL){
    # Sample locations
...
results <- lapply(c(FALSE), function(bool) sapply(1:M, function(i)
...
sapply(1:M, function(i) rmse(df_list[[i]]$Linf_i, results[[1]][,i]))
sapply(1:M, function(i) rmse(df_list[[i]]$Linf_i, results[[2]][,i]))
