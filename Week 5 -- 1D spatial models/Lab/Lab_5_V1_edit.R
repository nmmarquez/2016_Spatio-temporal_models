...
library(TMB)
setwd( "~/Documents/Classes/2016_Spatio-temporal_models/Week 5 -- 1D spatial models/Lab/" )
...
loc_s[1] = 0
for(s in 2:length(x)) {
    loc_s[s] = loc_s[s-1] + rlnorm(1, meanlog=0, sdlog=1)
}
...
for(s in 2:length(x)) {
    epsilon_s[s] = Rho^abs(loc_s[s]-loc_s[s-1]) * epsilon_s[s-1] + 
        rnorm(1, mean=0, sd=sqrt(Sigma2*(1-Rho^(2*abs(loc_s[s]-loc_s[s-1])))))
}
# SImulate counts
...
Start_time = Sys.time()
Obj = MakeADFun( data=Data, parameters=Params, random="epsilon_s", DLL=model_name )
# Optimize
