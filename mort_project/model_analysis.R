...
run_model <- function(df, option=1, model_name=model, print=F){
    ...
    Params <- list(gpz=rep(0, par_num), log_sigma_obs=0, logit_rho_time=0,
                   logit_rho_age=0, logit_rho_loc=0, log_sigma_time=0,
...
silder2 <- run_model(df, option=1, print=T)

# get the global values to use for simulation
df$log_rate_hat <- silder2$log_rate_mort_hat
time_plot(subset(df, loc_group==3), 1, preds=T)
age_plot(subset(df, loc_group==3), 1, preds=T)
silder2$phi
