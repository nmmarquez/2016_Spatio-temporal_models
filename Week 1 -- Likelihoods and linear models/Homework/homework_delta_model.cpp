...
Type objective_function<Type>::operator() ()
{
  ...
  for( int i=0; i<n_data; i++){
    ...
    if(y_i(i)!=0 & Options_z(0)==0) jnll_i(i) -= log( 1-zero_prob ) + dlognorm( y_i(i), linpred_i(i), logsd, true );
    if(y_i(i)!=0 & Options_z(0)==1) jnll_i(i) -= log( 1-zero_prob ) + dgamma( y_i(i), pow(logsd,-2), exp(linpred_i(i))*pow(logsd,2), true );
    if(y_i(i)!=0 & Options_z(0)==2) /// Add something here
    // Running counter
